...
#This scripts creates topologies as CDP sees it with links blocked by STP removed. If no ports are blocked for a VLAN the topology is the same as the Physical topology, this is to avoid creating a topology for each VLAN when all are the same(Like for VLAN 1002-1005 that is not really used anymore). The STP root for the VLAN is shown for the VLAN topologies.
import os
import graphviz
...
#The reason for this is that the tuples ('R1.AnsibleLab', 'Ser 1/2') and ('R1.AnsibleLab', ' Ser 1/2') are different due to the "Ser 1/2" and " Ser 1/2" being different. 
    ...
    RemoteLink[:]=[item for item in RemoteLink if (item!='' and item!="WS-C3750-")]#This is to avoid issues with C3750 taking up a lot of space in show cdp neighbor output and then the parser of the files do not understand as there is only one space between C3750- and the Interface name.
...
#Loops through all the lines in the file.  The interface names in the file holds blocking interfaces meaning they should be removed from the topology.
  for Lines in CurrentFileLines:
    ...
    CurrentLineTemp=Lines.replace('\n','').split("  ") #This is to remove all new line signs in the list.
    CurrentLineTemp=Lines.replace(',','  ').split("  ") #This is to remove the , between multiple blocked ports if there are any.
    CurrentLineTemp[:]=[item for item in CurrentLineTemp if item!='']
#Below if-statements are to fix the output as CDP ses Gig/Fas but STP only sees Gi/Fa as the start of the interface name. CurrentLineTemp[0] is the VLAN number and CurrentLineTemp[1] is the interface that is blocked in STP. It loops through as it can be multiple ports blocked in one VLAN.
    y=0
    while y<len(CurrentLineTemp):
      if CurrentLineTemp[y][0:3]==' Fa':
        CurrentLineTemp[y]=CurrentLineTemp[y][1:3]+'s'+CurrentLineTemp[y][3:]
      elif CurrentLineTemp[y][0:3]==' Gi':
        CurrentLineTemp[y]=CurrentLineTemp[y][1:3]+'g'+CurrentLineTemp[y][3:]
#      print CurrentLineTemp[y]
#Below is the varible that holds the hostname and interface link same way as in edges dictionary. 
      comparing=(CurrentNode,CurrentLineTemp[y])
...
#Next section is to figure out the root of the Spanning tree and remember that label so it shows in the VLAN STP diagram.
#Exception to check if there even are any files in the directory.
try:
  ListOfFileNamesSTPRoot=os.listdir('./STPRoot')
except:
  ListOfFileNamesSTPRoot=""
...
for FileName in ListOfFileNamesSTPRoot:
  ...
  CurrentNode=FileName.split('_')[0]+".AnsibleLab"
#Opens the file and reads all the lines in the file.
  CurrentFile=open("STPRoot/"+FileName,"rt")
  CurrentFileLines=CurrentFile.readlines()
  i=0
#Below loop checks all lines for This bridge is the root and if so the previous element is the VLAN number the CurrentNode is root for. 
  for Lines in CurrentFileLines:
    CurrentLineTemp=Lines.replace('\n','').split("  ")
    CurrentLineTemp[:]=[item for item in CurrentLineTemp if item!='']
    if "This bridge is the root" in str(CurrentLineTemp):
      STPRootVLAN[CurrentFileLines[i-1].replace('\n','')]=CurrentNode
    i+=1
